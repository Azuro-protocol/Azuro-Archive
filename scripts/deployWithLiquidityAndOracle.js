...
async function main() {
    ...
    const oracle = "0xb00cD80067CFCBe389Ba94D050B54984D8A75c86";
    ...
    await timeout(8000);
    let azurobetImpl =  await proxyAdmin.getProxyImplementation(azurobet.address)
    ...
    let lpImpl =  await proxyAdmin.getProxyImplementation(lp.address)


    // CORE
    const Core = await ethers.getContractFactory("Core");
    const core = await upgrades.deployProxy(Core, [reinforcement, oracle, marginality, math.address]);
    await core.deployed();
    await timeout(8000);
    console.log("core deployed to:", core.address);
    let coreImpl =  await proxyAdmin.getProxyImplementation(core.address)
    ...
    //verification
    let mathImpl = await proxyAdmin.getProxyImplementation(math.address)
